<div align="center">

# 👋 Hello, I'm **Mariya**

const mariya = {
  building: ["AI", "UI", "cool things"],
  debugging: "probably",
  coffee: "needed, black!",
  interests: ["AI/ML", "LLMs", "UI/UX", "Full-Stack"],
  languages: ["English", "Hindi", "Marathi", "Urdu", "Turkish (Learning)"],
  motto: "build. break. learn. repeat."
};

<img src="https://readme-typing-svg.demolab.com?font=Fira+Code&size=21&duration=3000&pause=1000&color=7C3AED&center=true&vCenter=true&width=650&lines=Turning+ideas+into+working+products+%F0%9F%9A%80;Building+AI+that+solves+real+problems+%F0%9F%A4%96;Designing+interfaces+people+actually+enjoy+%F0%9F%8E%A8;Learning%2C+building%2C+breaking%2C+rebuilding+%E2%9C%A8" />

<br>

<a href="https://github.com/mariya0407">
<img src="https://img.shields.io/badge/GitHub-mariya0407-181717?style=for-the-badge&logo=github&logoColor=white"/>
</a>
<a href="https://www.linkedin.com/in/mariya-shaikh-29a04m/">
<img src="https://img.shields.io/badge/LinkedIn-Mariya%20Shaikh-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white"/>
</a>
<a href="mailto:mariyashaikh0407@gmail.com">
<img src="https://img.shields.io/badge/Email-Contact-EA4335?style=for-the-badge&logo=gmail&logoColor=white"/>
</a>

<br><br>

<img src="https://komarev.com/ghpvc/?username=mariya0407&style=for-the-badge&color=7C3AED&label=PROFILE+VIEWS"/>

</div>

---

<table>
<tr>
<td width="65%">

## 🧠 A little about me

I'm an **AI & Data Science undergraduate at VIIT, Pune**, interested in the space where **AI, software, design and creativity** meet.

I like taking an idea from:

`💡 Idea → 🎨 Design → 💻 Code → 🧠 Intelligence → 🚀 Product`

Currently exploring **LLMs, NLP, AI agents, voice assistants and human-centered interfaces.**

</td>

<td width="35%" align="center">

### currently

🤖 building with AI
🎨 designing interfaces
💻 writing code
🧪 breaking things
☕ probably debugging

</td>
</tr>
</table>

---

## 🧩 My Toolbox

<p align="center">

<img src="https://skillicons.dev/icons?i=python,cpp,js,ts,react,nextjs,fastapi,django,tensorflow,pytorch,mongodb,postgres,mysql,git,github,figma" />

</p>

<div align="center">

`AI/ML`   `LLMs`   `NLP`   `Full-Stack`   `UI/UX`   `Data`

</div>

---

## ✨ Things I've Built

<table>
<tr>

<td width="50%">

### ✨ Simplr

**AI Text Simplification Tool**

A Chrome extension that turns complicated webpage content into something easier to understand.

`React` `TypeScript` `FastAPI` `LangChain`

</td>

<td width="50%">

### 🩺 Medico

**AI Telemedicine Ecosystem**

A unified healthcare platform connecting patients, doctors and pharmacies with AI-powered features.

`AI` `Gemini` `Full-Stack`

</td>

</tr>

<tr>

<td width="50%">

### 🌾 AgriAI

**Crop & Livestock Intelligence**

A deep-learning platform for plant disease detection, soil classification and livestock identification.

`Python` `Deep Learning` `Streamlit`

</td>

<td width="50%">

### 🐾 Pawkits

**Smart Pet Collar**

A pet-care platform built around a connected collar concept for safety and monitoring.

`UI/UX` `Product Design` `Web`

</td>

</tr>
</table>

---

## 📊 GitHub, but make it data

<div align="center">

<img height="165" src="https://github-readme-stats.vercel.app/api?username=mariya0407&show_icons=true&hide_border=true&theme=transparent&rank_icon=github"/>

<img height="165" src="https://github-readme-stats.vercel.app/api/top-langs/?username=mariya0407&layout=compact&hide_border=true&theme=transparent"/>

</div>

---

## 🌱 Currently obsessed with

<p align="center">

`🧠 LLMs`   `🗣️ Voice AI`   `🔗 RAG`   `🤖 AI Agents`   `🌐 NLP`   `🎨 UI/UX`

</p>

---

<div align="center">

### `if (idea) { build(idea); }`

<br>

<a href="https://github.com/mariya0407">
<img src="https://img.shields.io/badge/Explore%20my%20work-181717?style=for-the-badge&logo=github&logoColor=white"/>
</a>

<a href="https://www.linkedin.com/in/mariya-shaikh-29a04m/">
<img src="https://img.shields.io/badge/Let's%20connect-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white"/>
</a>

<br><br>

<img src="https://capsule-render.vercel.app/api?type=waving&color=7C3AED&height=90&section=footer"/>

</div>
